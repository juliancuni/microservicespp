...
//Input JSON body to the request
void HttpRequestInput::add_json_body(QJsonObject json) {
    QJsonDocument jsonDoc(json);
    QByteArray vars= jsonDoc.toJson();
}
...
void HttpRequestWorker::execute(HttpRequestInput *input) {
    ...
    } else if (input->var_layout == JSON_DOC) {
        request_content = "";
    }
    else {
    ...
    request.setRawHeader("User-Agent", "Microservices for Desktop APP");

    if(input->var_layout == JSON_DOC) {
